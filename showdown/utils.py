...
require_coro_msg = (
    "{} must be defined as a couroutine (using async def). \n"
...
def require_client_session(func):
    ...
    Returns:
        A wrapped version of the function. The object client.session attribute
        will be passed in as the sessionkeyword if None is provided.
...
def clean_message_content(content, strict=False):
    ...
    if len(content) > 300:
        if strict:
            raise ValueError(
                "Message content is too long (>300 characters). The "
